<%-- $Id: ScreenLock.jsp 87 2008-10-04 03:07:38Z admin $ --%>
        ...
        <TR>
          <TD bgColor=#413a8a>
            <TABLE cellSpacing=0 cellPadding=0 bgColor=white border=0>
              ...
              <TR>
                <TD noWrap><h:label id="msg" templateKey="T_InJspMessage"/>&nbsp;</TD>
                <TD width=9><IMG height=1 src="<%=request.getContextPath()%>/img/common/spacer.gif" width=9 border=0></TD></TR>
              <TR height=12>
                <TD height=12 align=center><h:submitbutton id="btn_ToLoginScreen" templateKey="T_ToLoginScreen"/></TD>
...
<script>
function timerScript(messageText)
{
	document.getElementById('btn_ToLoginScreen').style.visibility='visible';
	document.getElementById('btn_ToLoginScreen').disabled=false;
...
function unLockMainMenu()
{
	try
	{
		window.parent.frame1.wSubmitFlag = true;
	}
	catch(e)
	{
	}
...
function lockMainMenu()
{
	try
	{
		window.parent.frame1.wSubmitFlag = false;
	}
	catch(e)
	{
	}
