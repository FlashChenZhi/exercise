<%-- $Id: RetrievalOrderAllocClear.jsp 7134 2010-02-18 07:39:31Z shibamoto $ --%>
...
<h:head>
  <script>
	function dynamicAction()
	{
		...
		{
			if (!_isValueInput('txt_RetrievalPlanDate'))
			{
				document.all('txt_RetrievalPlanDate').focus();
				// 6003100={0}を入力してください。
				_setMessage('<%= HTMLUtils.escapeScript(HTMLUtils.encode(MessageResources.getText("6003100", DispResources.getText("LBL-W0127")))) %>',2);
				return true;
			}
			
			if (_isValueInput('txt_OrderNoFrom') && _isValueInput('txt_OrderNoTo'))
			{
				if (!_isCorrectRange(document.all('txt_OrderNoFrom').value, document.all('txt_OrderNoTo').value))
				{
					document.all('txt_OrderNoTo').focus();
					// 6023051=開始オーダーNo.は、終了オーダーNo.より前のオーダーNoにしてください。
					_setMessage('<%= HTMLUtils.escapeScript(HTMLUtils.encode(MessageResources.getText("6023051"))) %>',2);
					return true;
				}
			}
			...
			if ((document.all('txt_OrderNoFrom').value).indexOf("*", 0) >= 0)
			{
				document.all('txt_OrderNoFrom').focus();
				_setMessage('<%= HTMLUtils.escapeScript(HTMLUtils.encode(MessageResources.getText("6003101", DispResources.getText("LBL-W0225")))) %>',2);
				return true;
			}
			
			if ((document.all('txt_OrderNoTo').value).indexOf("*", 0) >= 0)
			{
				document.all('txt_OrderNoTo').focus();
				_setMessage('<%= HTMLUtils.escapeScript(HTMLUtils.encode(MessageResources.getText("6003101", DispResources.getText("LBL-W0225")))) %>',2);
				return true;
      ...
      border=0>
        ...
        <TR height=28>
          <TD width=7><IMG 
            ...
            width=7></TD>
          <TD width=7></TD>
          <TD noWrap><h:tab id="tab" templateKey="W_RetrievalCancel"/></TD>
        ...
        <TR>
          <TD>
            <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
              ...
              <TR bgColor=#b8b7d7>
                <TD noWrap bgColor=#b8b7d7><h:label id="lbl_OrderNoFrom" templateKey="W_OrderNo"/></TD>
                <TD bgColor=#b8b7d7><IMG 
                  src="<%=request.getContextPath()%>/img/common/icon_colon3.gif"></TD>
                <TD width="100%" bgColor=#b8b7d7><h:freetextbox id="txt_OrderNoFrom" templateKey="W_OrderNo"/><h:label id="lbl_Range" templateKey="W_Range"/><h:freetextbox id="txt_OrderNoTo" templateKey="W_OrderNo"/></TD></TR></TBODY></TABLE></TD></TR></TBODY></TABLE>
      <TABLE style="MARGIN-LEFT: 7px" cellSpacing=0 cellPadding=0 border=0>
        ...
        <TR>
          ...
          <TD bgColor=#b8b7d7>
            ...
            <h:submitbutton id="btn_Set" templateKey="W_Set2"/>&nbsp;&nbsp; 
        ...
        <TR>
          <TD>
            <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
              ...
              <TR bgColor=#b8b7d7>
                <TD noWrap bgColor=#b8b7d7></TD>
                <TD bgColor=#b8b7d7 colSpan=2><h:listcell id="lst_RetrievalAllocateWorkList" templateKey="W_RetrievalAllocateWorkList"/></TD></TR>
              <TR bgColor=#b8b7d7>
                <TD noWrap bgColor=#b8b7d7>&nbsp;&nbsp; </TD>
                <TD bgColor=#b8b7d7></TD>
                <TD width="100%" bgColor=#b8b7d7></TD></TR>
