<%-- $Id: ItemWorkingInquiry.jsp 2144 2008-12-15 09:25:05Z okayama $ --%>
 ...
 *
 * @version $Revision: 2144 $, $Date: 2008-12-15 18:25:05 +0900 (月, 15 12 2008) $
...
<h:head>
  <script>
    function dynamicAction()
    {
        ...
        {
            if (_isValueInput('txt_ItemCodeFrom') && _isValueInput('txt_ItemCodeTo'))
            {
                if (!_isCorrectRange(document.all('txt_ItemCodeFrom').value, document.all('txt_ItemCodeTo').value))
                {
                    document.all('txt_ItemCodeTo').focus();
                    _setMessage('<%= HTMLUtils.escapeScript(HTMLUtils.encode(MessageResources.getText("6023024"))) %>',2);
                    return true;
                }
            }
            if ((document.all('txt_ItemCodeFrom').value).indexOf("*", 0) >= 0)
            {
                document.all('txt_ItemCodeFrom').focus();
                _setMessage('<%= HTMLUtils.escapeScript(HTMLUtils.encode(MessageResources.getText("6003101", DispResources.getText("LBL-W0189")))) %>',2);
                return true;
            }

            if ((document.all('txt_ItemCodeTo').value).indexOf("*", 0) >= 0)
            {
                document.all('txt_ItemCodeTo').focus();
                _setMessage('<%= HTMLUtils.escapeScript(HTMLUtils.encode(MessageResources.getText("6003101", DispResources.getText("LBL-W0189")))) %>',2);
                return true;
          ...
          <TD>
            <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
              ...
              <TR bgColor=#b8b7d7>
                ...
                <TD width="100%" bgColor=#b8b7d7><h:freetextbox id="txt_ItemCodeFrom" templateKey="W_ItemCode"/><h:label id="lbl_Range" templateKey="W_Range"/><h:freetextbox id="txt_ItemCodeTo" templateKey="W_ItemCode"/></TD></TR>
              <TR bgColor=#b8b7d7>
              ...
              <TR bgColor=#b8b7d7>
                ...
                <TD bgColor=#b8b7d7><IMG src="<%=request.getContextPath()%>/img/common/icon_colon3.gif"></TD>
                <TD width="100%" bgColor=#b8b7d7><h:radiobutton id="rdo_GroupCondition_DetailUnit" templateKey="W_GroupCondition_DetailDisplay"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
                  <h:radiobutton id="rdo_GroupConditin_ItemUnit" templateKey="W_GroupCondition_ItemUnit"/></TD></TR></TBODY></TABLE></TD></TR></TBODY></TABLE>
        ...
        <TR>
          <TD>
            <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
              ...
              <TR bgColor=#b8b7d7>
                <TD noWrap bgColor=#b8b7d7>　　　　　　　</TD>
                <TD bgColor=#b8b7d7><h:listcell id="lst_StockByItemList" templateKey="W_StockByItemList"/></TD>
