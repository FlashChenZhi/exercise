...
import jp.co.daifuku.wms.stock.rft.schedule.StdAreaListSCH;
import jp.co.daifuku.wms.stock.rft.schedule.StdAreaListSCHParams;
 ...
 */
public class StdAreaListBusiness
        extends AbstractHSBusiness
{
    ...
    //------------------------------------------------------------
    /** SCREEN_NAME */
    private static final String SCREEN_NAME = "StdAreaList";
    ...
    public HSResult page_Load(DataSheetFactory dsf, Locale locale)
    {
        ...
        StdAreaListSCH sch = null;
        try
        {
            // open connection.
            conn = ConnectionManager.getConnection(WMSConstants.DATASOURCE_RFT_NAME);
            sch = new StdAreaListSCH(conn, this.getClass(), locale);

            // set input parameters.
            StdAreaListSCHParams inparam = new StdAreaListSCHParams();
            inparam.set(StdAreaListSCHParams.CONSIGNOR_CODE, receivingStorageInDataSheet.getValue(ReceivingStorageInDataSheet.CONSIGNOR_CODE));
            inparam.set(StdAreaListSCHParams.ITEM_CODE, receivingStorageInDataSheet.getValue(ReceivingStorageInDataSheet.ITEM_CODE));
            inparam.set(StdAreaListSCHParams.STORAGE_ITF_TO_JAN, termInfoDataSheet.getValue(TermInfoDataSheet.STORAGE_ITF_TO_JAN));
            ...
            for (int i = 0; i < outparams.size(); i++)
            {
                ...
                areaListDataSheet.setValue(AreaListDataSheet.AREA_NO, outparam.get(StdAreaListSCHParams.AREA_NO), i);
                areaListDataSheet.setValue(AreaListDataSheet.AREA_NAME, outparam.get(StdAreaListSCHParams.AREA_NAME), i);
            }
    ...
    public HSResult ent_Click(DataSheetFactory dsf, Locale locale)
    {
        ...
        try
        {
            ...
            inparam.set(StdStartSCHParams.AREA_NO, areaListDataSheet.getValue(AreaListDataSheet.AREA_NO));
            inparam.set(StdStartSCHParams.LOCATION_NO, WmsParam.DEFAULT_LOCATION_NO);
