...
import jp.co.daifuku.wms.receiving.rft.schedule.RecPlanDateListSCH;
import jp.co.daifuku.wms.receiving.rft.schedule.RecPlanDateListSCHParams;
 ...
 */
public class RecPlanDateListBusiness
        extends AbstractHSBusiness
{
    ...
    //------------------------------------------------------------
    /** SCREEN_NAME */
    private static final String SCREEN_NAME = "RecPlanDateList";
     ...
     * Default constructor
     */
    public RecPlanDateListBusiness()
    {
        super();
    }
    ...
    public HSResult page_Load(DataSheetFactory dsf, Locale locale)
    {
        ...
        RecPlanDateListSCH sch = null;
        try
        {
            // open connection.
            conn = ConnectionManager.getConnection(WMSConstants.DATASOURCE_RFT_NAME);
            sch = new RecPlanDateListSCH(conn, this.getClass(), locale);

            // set input parameters.
            RecPlanDateListSCHParams inparam = new RecPlanDateListSCHParams();
            inparam.set(RecPlanDateListSCHParams.TERMINAL_NO, termInfoDataSheet.getValue(TermInfoDataSheet.TERMINAL_NO));
            inparam.set(RecPlanDateListSCHParams.RECEIVING_ITF_TO_JAN, termInfoDataSheet.getValue(TermInfoDataSheet.RECEIVING_ITF_TO_JAN));
            inparam.set(RecPlanDateListSCHParams.USER_ID, commonDataSheet.getValue(CommonDataSheet.USER_ID));
            inparam.set(RecPlanDateListSCHParams.CONSIGNOR_CODE, receivingInDataSheet.getValue(ReceivingInDataSheet.CONSIGNOR_CODE));
            inparam.set(RecPlanDateListSCHParams.ITEM_CODE, receivingInDataSheet.getValue(ReceivingInDataSheet.ITEM_CODE));
            ...
            for (int i = 0; i < outparams.size(); i++)
            {
                ...
                planDateListDataSheet.setValue(PlanDateListDataSheet.PLAN_DATE, outparam.get(RecPlanDateListSCHParams.PLAN_DATE), i);
            }
    ...
    public HSResult ent_Click(DataSheetFactory dsf, Locale locale)
    {
        ...
        try
        {
            ...
            inparam.set(RecStartSCHParams.PLAN_DATE, planDateListDataSheet.getValue(PlanDateListDataSheet.PLAN_DATE));
