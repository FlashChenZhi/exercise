...
import jp.co.daifuku.wms.base.rft.datasheet.PlanDateListDataSheet;
...
import jp.co.daifuku.wms.ship.rft.schedule.ShpPlanDateListSCH;
import jp.co.daifuku.wms.ship.rft.schedule.ShpPlanDateListSCHParams;
 ...
 */
public class ShpPlanDateListBusiness
        extends AbstractHSBusiness
{
    ...
    //------------------------------------------------------------
    /** SCREEN_NAME */
    private static final String SCREEN_NAME = "ShpPlanDateList";
     ...
     * Default constructor
     */
    public ShpPlanDateListBusiness()
    {
        super();
    }
    ...
    public HSResult page_Load(DataSheetFactory dsf, Locale locale)
    {
        ...
        PlanDateListDataSheet planDateListDataSheet = new PlanDateListDataSheet(dsf, locale);
        ...
        ShpPlanDateListSCH sch = null;
        try
        {
            // open connection.
            conn = ConnectionManager.getConnection(WMSConstants.DATASOURCE_RFT_NAME);
            sch = new ShpPlanDateListSCH(conn, this.getClass(), locale);

            // set input parameters.
            ShpPlanDateListSCHParams inparam = new ShpPlanDateListSCHParams();
            inparam.set(ShpPlanDateListSCHParams.TERMINAL_NO, termInfoDataSheet.getValue(TermInfoDataSheet.TERMINAL_NO));
            inparam.set(ShpPlanDateListSCHParams.USER_ID, commonDataSheet.getValue(CommonDataSheet.USER_ID));
            inparam.set(ShpPlanDateListSCHParams.CONSIGNOR_CODE, shippingCustomerInDataSheet.getValue(ShippingCustomerInDataSheet.CONSIGNOR_CODE));
            inparam.set(ShpPlanDateListSCHParams.BATCH_NO, shippingCustomerInDataSheet.getValue(ShippingCustomerInDataSheet.BATCH_NO));
            inparam.set(ShpPlanDateListSCHParams.CUSTOMER_CODE, shippingCustomerInDataSheet.getValue(ShippingCustomerInDataSheet.CUSTOMER_CODE));

            // SCH call.
            List<Params> outparams = sch.query(inparam);

            // set output parameters.
            planDateListDataSheet.setValue(DataSheetModel.SIZE, outparams.size());
            for (int i = 0; i < outparams.size(); i++)
            {
                Params outparam = outparams.get(i);
                planDateListDataSheet.setValue(PlanDateListDataSheet.PLAN_DATE, outparam.get(ShpPlanDateListSCHParams.PLAN_DATE), i);
            }

            // initialize DataSheet view.
            planDateListDataSheet.initView();
    ...
    public HSResult ent_Click(DataSheetFactory dsf, Locale locale)
    {
        ...
        try
        {
            ...
            inparam.set(ShpStartSCHParams.PLAN_DATE, planDateListDataSheet.getValue(PlanDateListDataSheet.PLAN_DATE));
