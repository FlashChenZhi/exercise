...
import jp.co.daifuku.wms.stock.rft.schedule.StdLotNoListSCH;
import jp.co.daifuku.wms.stock.rft.schedule.StdLotNoListSCHParams;
 ...
 */
public class StdLotNoListBusiness
        extends AbstractHSBusiness
{
    ...
    //------------------------------------------------------------
    /** SCREEN_NAME */
    private static final String SCREEN_NAME = "StdLotNoList";
    ...
    public HSResult page_Load(DataSheetFactory dsf, Locale locale)
    {
        ...
        StdLotNoListSCH sch = null;
        try
        {
            // open connection.
            conn = ConnectionManager.getConnection(WMSConstants.DATASOURCE_RFT_NAME);
            sch = new StdLotNoListSCH(conn, this.getClass(), locale);

            // set input parameters.
            StdLotNoListSCHParams inparam = new StdLotNoListSCHParams();
            inparam.set(StdLotNoListSCHParams.CONSIGNOR_CODE, receivingStorageInDataSheet.getValue(ReceivingStorageInDataSheet.CONSIGNOR_CODE));
            inparam.set(StdLotNoListSCHParams.ITEM_CODE, receivingStorageInDataSheet.getValue(ReceivingStorageInDataSheet.ITEM_CODE));
            inparam.set(StdLotNoListSCHParams.AREA_NO, receivingStorageInDataSheet.getValue(ReceivingStorageInDataSheet.AREA_NO));
            inparam.set(StdLotNoListSCHParams.LOCATION_NO, receivingStorageInDataSheet.getValue(ReceivingStorageInDataSheet.LOCATION_NO));
            inparam.set(StdLotNoListSCHParams.STORAGE_ITF_TO_JAN, termInfoDataSheet.getValue(TermInfoDataSheet.STORAGE_ITF_TO_JAN));
            ...
            for (int i = 0; i < outparams.size(); i++)
            {
                ...
                lotNoListDataSheet.setValue(LotNoListDataSheet.LOT_NO, outparam.get(StdLotNoListSCHParams.LOT_NO), i);
            }
    ...
    public HSResult ent_Click(DataSheetFactory dsf, Locale locale)
    {
        ...
        try
        {
            ...
            inparam.set(StdStartSCHParams.AREA_NO, receivingStorageInDataSheet.getValue(ReceivingStorageInDataSheet.AREA_NO));
            inparam.set(StdStartSCHParams.LOCATION_NO, receivingStorageInDataSheet.getValue(ReceivingStorageInDataSheet.LOCATION_NO));
            inparam.set(StdStartSCHParams.LOT_NO, lotNoListDataSheet.getValue(LotNoListDataSheet.LOT_NO));
            inparam.set(StdStartSCHParams.LOT_NO_SELECT, true);
