...
import jp.co.daifuku.wms.stock.rft.schedule.NrtLotNoListSCH;
import jp.co.daifuku.wms.stock.rft.schedule.NrtLotNoListSCHParams;
 ...
 */
public class NrtLotNoListBusiness
        extends AbstractHSBusiness
{
    ...
    //------------------------------------------------------------
    /** SCREEN_NAME */
    private static final String SCREEN_NAME = "NrtLotNoList";
    ...
    public HSResult page_Load(DataSheetFactory dsf, Locale locale)
    {
        ...
        NrtLotNoListSCH sch = null;
        try
        {
            // open connection.
            conn = ConnectionManager.getConnection(WMSConstants.DATASOURCE_RFT_NAME);
            sch = new NrtLotNoListSCH(conn, this.getClass(), locale);

            // set input parameters.
            NrtLotNoListSCHParams inparam = new NrtLotNoListSCHParams();
            inparam.set(NrtLotNoListSCHParams.CONSIGNOR_CODE, noPlanRetrievalInDataSheet.getValue(NoPlanRetrievalInDataSheet.CONSIGNOR_CODE));
            inparam.set(NrtLotNoListSCHParams.AREA_NO, noPlanRetrievalInDataSheet.getValue(NoPlanRetrievalInDataSheet.AREA_NO));
            inparam.set(NrtLotNoListSCHParams.LOCATION_NO, noPlanRetrievalInDataSheet.getValue(NoPlanRetrievalInDataSheet.LOCATION_NO));
            inparam.set(NrtLotNoListSCHParams.ITEM_CODE, noPlanRetrievalInDataSheet.getValue(NoPlanRetrievalInDataSheet.ITEM_CODE));
            inparam.set(NrtLotNoListSCHParams.NO_PLAN_RETRIEVAL_ITF_TO_JAN, termInfoDataSheet.getValue(TermInfoDataSheet.NO_PLAN_RETRIEVAL_ITF_TO_JAN));
            ...
            for (int i = 0; i < outparams.size(); i++)
            {
                ...
                lotNoListDataSheet.setValue(LotNoListDataSheet.LOT_NO, outparam.get(NrtLotNoListSCHParams.LOT_NO), i);
            }
    ...
    public HSResult ent_Click(DataSheetFactory dsf, Locale locale)
    {
        ...
        try
        {
            ...
            inparam.set(NrtStartSCHParams.LOT_NO, lotNoListDataSheet.getValue(LotNoListDataSheet.LOT_NO));
            inparam.set(NrtStartSCHParams.LOT_NO_SELECT, true);
