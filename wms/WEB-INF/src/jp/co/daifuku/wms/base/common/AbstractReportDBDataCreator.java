// $Id: AbstractReportDBDataCreator.java 7733 2010-03-26 06:21:56Z okayama $
 ...
 */
public abstract class AbstractReportDBDataCreator
        implements ReportDBDataCreator, ScheduleJob
{
    //------------------------------------------------------------
    ...
    private static int $reportSequence = 0; //database wants timestamp or sequence to be unique across all message types
    ...
    private HostSendHandler _HostSendHandler = null;
     ...
     */
    protected AbstractReportDBDataCreator(Connection conn, Connection customerConn)
    {
        super();
        setConnection(conn);
        setCustomerConnection(customerConn);
        _HostSendHandler = new HostSendHandler(getConnection());
    }
     ...
     */
    protected AbstractReportDBDataCreator(Connection conn, Connection customerConn, Class caller)
    {
        super();
        setConnection(conn);
        setCustomerConnection(customerConn);
    ...
    public void execute(String[] args)
    {
        synchronized ($mutex)
        {
            ...
            try
            {
                ...
                Connection customerConn = WmsParam.getCustomerConnection();
                setConnection(conn);
                setCustomerConnection(customerConn);
                ...
                ReportDBDataCreator creator = getReportClass();
     ...
     */
    protected boolean report(Connection conn, ReportDBDataCreator creator)
            throws IOException,
                ReadWriteException
    {
        // 実績データの作成
     ...
     */
    protected int getNextInReportSequence()
    {
        $reportSequence = $reportSequence + 1;
        if ($reportSequence > 999999) //SEQUENCE_NO field in the DB is 6 digits
        {
            $reportSequence = 1;
        }
     ...
     */
    public void setResultFileName(String resutlFileName)
    {
        _resultFileName = resutlFileName;
    }
    ...
    /**
     * Set the connection to the customer's database<br>
     * @param conn customer's database connection<br>
     */
    ...
    /**
     * Get the connection to the customer's database<br>
     * @return customer's database connection<br>
     */
    ...
    protected void moveErrorFilesToHistory()
    {
        ...
        {
            String newFileName = replaceExt(errFile.getName(), getExtention());
    ...
    /**
     * WMSParam環境ファイルから報告データの環境情報を取得します。<br>
     * @param reportDataType 報告データのデータタイプ<br>
     * @throws ScheduleException スケジュール処理の実行中に予期しない例外が発生した場合に通知されます。<br>
     */
    protected void acquireFileInfo(String reportDataType)
            throws ScheduleException
    {
        try
        {
            // データ区分セット
            setExchangeJob(reportDataType);
            // 送受信開始日時セット
            ...
            // 検索条件セット
            key.setJobType(reportDataType);
            key.setExchangeType(ExchangeEnvironment.EXCHANGE_TYPE_SEND);
            key.setDataType(ExchangeEnvironment.DATA_DB);
            ...
            if (env == null)
            {
                ...
                RmiMsgLogClient.write(new TraceHandler(6026004, new ScheduleException()), "AbstractReportDBDataCreator");
                throw new ScheduleException();
            }
            else if (StringUtil.isBlank(env.getDataId()))
            {
        ...
        {
            // 6026005=環境定義情報が見つかりませんでした。
            RmiMsgLogClient.write(new TraceHandler(6026005, ex), "AbstractReportDBDataCreator");
            throw new ScheduleException();
        }
        catch (NoPrimaryException ex)
        {
            ...
            RmiMsgLogClient.write(new TraceHandler(6027006, ex), "AbstractReportDBDataCreator", msg);
            throw new ScheduleException();
        }
        catch (InvalidDefineException ex)
        {
            // 6027013=不正なパラメータが検出されたため、処理を中断しました。
            RmiMsgLogClient.write(new TraceHandler(6027013, ex), "AbstractReportDBDataCreator");
            throw new ScheduleException();
        }
    }
                ...
                ReadWriteException
    {
        ...
        _HostSendHandler.modify(altKey);
    ...
    {
        return "$Id: AbstractReportDBDataCreator.java 7733 2010-03-26 06:21:56Z okayama $";
    }
}
