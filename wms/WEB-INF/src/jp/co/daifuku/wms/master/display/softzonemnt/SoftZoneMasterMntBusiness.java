...
/**
 * ソフトゾーンメンテナンス（ソフトゾーン範囲）の画面処理を行います。
 *
        ...
        extends SoftZoneMasterMnt
{
            ...
            throws Exception
    {
        ...
        try
        {
            ...
            //DFKLOOK:ここから修正
            // 第3引数(商品コード)：空文字、第4引数(フリー可否)：falseを渡す
            // load pul_SoftZoneId.
            _pdm_pul_SoftZone.init(conn, SoftZoneType.AREA, "", false);
            //DFKLOOK:ここまで修正
            ...
            throws Exception
    {
        ...
        try
        {
            ...
            // DFKLOOK:ここから修正
            // バンク(入れ替え)
            int from = Integer.parseInt(txt_BankFrom.getText().trim());
            int to = Integer.parseInt(txt_BankTo.getText().trim());
            if (from > to)
            {
                inparam.set(SoftZoneMasterMntSCHParams.BANK_FROM, String.valueOf(to));
                inparam.set(SoftZoneMasterMntSCHParams.BANK_TO, String.valueOf(from));
                txt_BankFrom.setValue(String.valueOf(to));
                txt_BankTo.setValue(String.valueOf(from));
            }
            else
            {
                inparam.set(SoftZoneMasterMntSCHParams.BANK_FROM, String.valueOf(from));
                inparam.set(SoftZoneMasterMntSCHParams.BANK_TO, String.valueOf(to));
                txt_BankFrom.setValue(String.valueOf(from));
                txt_BankTo.setValue(String.valueOf(to));
            }

            // ベイ(入れ替え)
            from = Integer.parseInt(txt_BayFrom.getText().trim());
            to = Integer.parseInt(txt_BayTo.getText().trim());
            if (from > to)
            {
                inparam.set(SoftZoneMasterMntSCHParams.BAY_FROM, String.valueOf(to));
                inparam.set(SoftZoneMasterMntSCHParams.BAY_TO, String.valueOf(from));
                txt_BayFrom.setValue(String.valueOf(to));
                txt_BayTo.setValue(String.valueOf(from));
            }
            else
            {
                inparam.set(SoftZoneMasterMntSCHParams.BAY_FROM, String.valueOf(from));
                inparam.set(SoftZoneMasterMntSCHParams.BAY_TO, String.valueOf(to));
                txt_BayFrom.setValue(String.valueOf(from));
                txt_BayTo.setValue(String.valueOf(to));
            }

            // レベル(入れ替え)
            from = Integer.parseInt(txt_LevelFrom.getText().trim());
            to = Integer.parseInt(txt_LevelTo.getText().trim());
            if (from > to)
            {
                inparam.set(SoftZoneMasterMntSCHParams.LEVEL_FROM, String.valueOf(to));
                inparam.set(SoftZoneMasterMntSCHParams.LEVEL_TO, String.valueOf(from));
                txt_LevelFrom.setValue(String.valueOf(to));
                txt_LevelTo.setValue(String.valueOf(from));
            }
            else
            {
                inparam.set(SoftZoneMasterMntSCHParams.LEVEL_FROM, String.valueOf(from));
                inparam.set(SoftZoneMasterMntSCHParams.LEVEL_TO, String.valueOf(to));
                txt_LevelFrom.setValue(String.valueOf(from));
                txt_LevelTo.setValue(String.valueOf(to));
            }
            // DFKLOOK:ここまで修正
            ...
            // DFKLOOK:ここから修正
            // "xxx - xxx"の形式に変換
            line.setValue(KEY_LST_BANK, sch.fromToFormat(txt_BankFrom.getText(), txt_BankTo.getText()));
            line.setValue(KEY_LST_BAY, sch.fromToFormat(txt_BayFrom.getText(), txt_BayTo.getText()));
            line.setValue(KEY_LST_LEVEL, sch.fromToFormat(txt_LevelFrom.getText(), txt_LevelTo.getText()));
            // DFKLOOK:ここまで修正
            ...
            throws Exception
    {
        ...
        // DFKLOOK:ここから修正
        // 行取消を行いリストが0行になる場合
        if (lst_ZoneMaintenanceRange.getMaxRows() == 1)
        {
            btn_Set.setEnabled(false);
            btn_AllClear.setEnabled(false);
        }
