// $Id: ReplenishListCompleteBusiness.java 7598 2010-03-16 01:30:35Z ota $
...
/**
 * 補充リスト作業完了設定の画面処理を行います。
 *
 * @version $Revision: 7598 $, $Date: 2010-03-16 10:30:35 +0900 (火, 16 3 2010) $
        ...
        extends ReplenishListComplete
{
    ...
    private static final ListCellKey KEY_LST_TO_LOCATION = new ListCellKey("LST_TO_LOCATION", new StringCellColumn(), true, false);
     ...
     * @throws Exception
     */
    public void lst_ReplenishmentResultInput_Click(ActionEvent e)
            throws Exception
    {
    }
     ...
     * @throws Exception
     */
    public void lst_ReplenishmentResultInput_ColumClick(ActionEvent e)
            throws Exception
    {
    }
            ...
            throws Exception
    {
        ...
        try
        {
            ...
            {
                inparam.set(ReplenishListCompleteSCHParams.SELECT_JOB_TYPE, SystemDefine.JOB_TYPE_NORMAL_REPLENISHMENT);
            }
            else
            {
                inparam.set(ReplenishListCompleteSCHParams.SELECT_JOB_TYPE, SystemDefine.JOB_TYPE_EMERGENCY_REPLENISHMENT);
            }
            // DFKLOOK end
            ...
            // DFKLOOK start 対象データなしの処理追加
            if (outparams.size() == 0)
            {
                btn_Complete.setEnabled(false);
                btn_AllClear.setEnabled(false);
                txt_LListWorkNo.setText("");
                txt_LRWorkFlag.setText("");
                return;
            }
            // DFKLOOK end 対象データなしの処理追加
            ...
            for (Params outparam : outparams)
            {
                ...
                if (rdo_ReplenishmentWorkFlagPlan.getChecked())
                {
                    viewState.setObject(ViewStateKeys.SELECT_JOB_TYPE, SystemDefine.JOB_TYPE_NORMAL_REPLENISHMENT);
                    line.setValue(KEY_LST_WORK_TYPE, SystemDefine.JOB_TYPE_NORMAL_REPLENISHMENT);
                }
                else
                {
                    viewState.setObject(ViewStateKeys.SELECT_JOB_TYPE, SystemDefine.JOB_TYPE_EMERGENCY_REPLENISHMENT);
                    line.setValue(KEY_LST_WORK_TYPE, SystemDefine.JOB_TYPE_EMERGENCY_REPLENISHMENT);
                }
                ...
                // DFKLOOK modify
                txt_LRWorkFlag.setValue(DispResources.getText(rdo_ReplenishmentWorkFlagPlan.getSelectedItem().getResourceKey()));
...
//                }
                ...
                // caseQty * casePack + pieceQty
                int casePack = WmsFormatter.getInt(lst_ReplenishmentResultInput.getValue(4));
                int caseQty = WmsFormatter.getInt(lst_ReplenishmentResultInput.getValue(6));
                int pieceQty = WmsFormatter.getInt(lst_ReplenishmentResultInput.getValue(11)); 
                lineparam.set(ReplenishListCompleteSCHParams.REPLENISH_QTY, casePack * caseQty + pieceQty);
                // DFKLOOK end
            ...
            // DFKLOOK start データなしの場合のみ処理を行うよう修正
            if (outparams.size() == 0)
            {
                // clear.
                btn_Complete.setEnabled(false);
