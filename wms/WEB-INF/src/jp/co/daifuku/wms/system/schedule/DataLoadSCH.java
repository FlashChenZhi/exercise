// $Id: DataLoadSCH.java 7241 2010-02-26 05:32:14Z okayama $
...
import static jp.co.daifuku.wms.system.schedule.DataLoadSCHParams.*;
        ...
        extends AbstractSCH
{
            ...
            throws CommonException
    {
        ...
        // Pカートパッケージが導入されている場合のみ
        if (sysCtrl.hasPCTRetrievalPack())
        {
            // プレフィックス指定のみ取得
            skey.setIsPrefix(ExchangeEnvironment.IS_PREFIX_TRUE);
        }
        ...
        for (ExchangeEnvironment exEnv : exEnvs)
        {
            ...
            // ファイル数
            param.set(FILES_COUNT, getLoadFileCount(exEnv.getClassName(), exEnv.getJobType()));
            // クラス名
            ...
            throws CommonException
    {
        ...
        try
        {
            ...
            for (ScheduleParams p : ps)
            {
                ...
                // ファイル数
                outParam.set(FILES_COUNT, getLoadFileCount(p.getString(CLASS_NAME), p.getString(DATA_TYPE)));
                // クラス名
     ...
     * @return 検出したファイル数
     */
    protected int getLoadFileCount(String className, String exchangeJob)
            throws CommonException
    {
        // システム定義コントローラ
        WarenaviSystemController sysCtrl = new WarenaviSystemController(getConnection(), this.getClass());
        ...
        try
        {
            ...
            if (sysCtrl.hasPCTRetrievalPack())
            {
                ...
                // インスタンス生成
                obj = constructor.newInstance(exchangeJob, getUserInfo(), getLocale());
            }
            else
            {
                // コンストラクタを取得
