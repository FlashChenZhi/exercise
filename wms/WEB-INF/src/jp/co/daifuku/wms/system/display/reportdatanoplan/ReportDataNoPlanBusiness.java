// $Id: ReportDataNoPlanBusiness.java 7996 2011-07-06 00:52:24Z kitamaki $
package jp.co.daifuku.wms.system.display.reportdatanoplan;
...
import jp.co.daifuku.wms.system.schedule.ReportDataNoPlanSCH;
import jp.co.daifuku.wms.system.schedule.ReportDataNoPlanSCHParams;
...
public class ReportDataNoPlanBusiness
        extends ReportDataNoPlan
{
     ...
     * Default constructor
     */
    public ReportDataNoPlanBusiness()
    {
        super();
    }
            ...
            throws Exception
    {
        ...
        ReportDataNoPlanSCH sch = null;
        try
        {
            // open connection.
            conn = ConnectionManager.getRequestConnection(this);
            sch = new ReportDataNoPlanSCH(conn, this.getClass(), locale, ui);

            // set input parameters.
            ReportDataNoPlanSCHParams inparam = new ReportDataNoPlanSCHParams();
            ...
            for (Params outparam : outparams)
            {
                ...
                line.setValue(KEY_HIDDEN_DATA_TYPE, outparam.get(ReportDataNoPlanSCHParams.DATA_TYPE));
                line.setValue(KEY_HIDDEN_CLASS_NAME, outparam.get(ReportDataNoPlanSCHParams.CLASS_NAME));
                line.setValue(KEY_LST_REPORT_DATA_TYPE, outparam.get(ReportDataNoPlanSCHParams.REPORT_DATA_TYPE));
                line.setValue(KEY_LST_MESSAGE, outparam.get(ReportDataNoPlanSCHParams.MESSAGE));
            ...
            throws Exception
    {
        ...
        try
        {
            ...
            for (int i = 1; i <= _lcm_lst_NoPlanReport.size(); i++)
            {
                ...
                ReportDataNoPlanSCHParams lineparam = new ReportDataNoPlanSCHParams();
                lineparam.setProcessFlag(ProcessFlag.REGIST);
                lineparam.setRowIndex(i);
                lineparam.set(ReportDataNoPlanSCHParams.DATA_TYPE, line.getValue(KEY_HIDDEN_DATA_TYPE));
                lineparam.set(ReportDataNoPlanSCHParams.CLASS_NAME, line.getValue(KEY_HIDDEN_CLASS_NAME));
                lineparam.set(ReportDataNoPlanSCHParams.SELECT, line.getValue(KEY_LST_SELECT));
                lineparam.set(ReportDataNoPlanSCHParams.REPORT_DATA_TYPE, line.getValue(KEY_LST_REPORT_DATA_TYPE));
            ...
            for (int i = 1; i <= _lcm_lst_NoPlanReport.size(); i++)
            {
                ...
                //                    continue;
                //                }
                // チェックされていないデータは対象外とする
                if (!lst_NoPlanReport.getChecked(_lcm_lst_NoPlanReport.getColumnIndex(KEY_LST_SELECT)))
                {
                    continue;
                }
                // 取り込みを行えていない場合も対象外とする
                if (!outparams.get(i - 1).getBoolean(ReportDataNoPlanSCHParams.LOG_FLAG))
                {
                	continue;
                }
                //DFKLOOK:ここまで修正
            ...
            for (Params outparam : outparams)
            {
                ListCellLine line = _lcm_lst_NoPlanReport.getNewLine();
                line.setValue(KEY_HIDDEN_DATA_TYPE, outparam.get(ReportDataNoPlanSCHParams.DATA_TYPE));
                line.setValue(KEY_HIDDEN_LOG_FLAG, outparam.get(ReportDataNoPlanSCHParams.LOG_FLAG));
                line.setValue(KEY_HIDDEN_CLASS_NAME, outparam.get(ReportDataNoPlanSCHParams.CLASS_NAME));
                line.setValue(KEY_LST_SELECT, outparam.get(ReportDataNoPlanSCHParams.SELECT));
                line.setValue(KEY_LST_REPORT_DATA_TYPE, outparam.get(ReportDataNoPlanSCHParams.REPORT_DATA_TYPE));
                line.setValue(KEY_LST_MESSAGE, outparam.get(ReportDataNoPlanSCHParams.MESSAGE));
                lst_NoPlanReport_SetLineToolTip(line);
                _lcm_lst_NoPlanReport.add(line);
