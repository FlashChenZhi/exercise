        ...
        extends ReprintOperation
{
    ...
    private ListCellModel _lcm_lst_ReprintOperation;
     ...
     * @throws Exception
     */
    public void lst_ReprintOperation_Click(ActionEvent e)
            throws Exception
    {
        //DFKLOOK:ここから修正
        // get event source column.
        int activeCol = lst_ReprintOperation.getActiveCol();
        
        // choose process.
        if (_lcm_lst_ReprintOperation.getColumnIndex(KEY_LST_PRINT_START) == activeCol)
        {
            // 印刷開始処理を呼び出し
            lst_PrintStart_Click_Process();
        }
        //DFKLOOK:ここまで修正
    }
            ...
            throws Exception
    {
        ...
        _lcm_lst_ReprintOperation = new ListCellModel(lst_ReprintOperation, LST_REPRINTOPERATION_KEYS, locale);
     ...
     * @throws Exception
     */
    private void lst_PrintStart_Click_Process()
            throws Exception
    {
        // 印刷開始ボタンを押下された行データを取得
        int row = lst_ReprintOperation.getActiveRow();
        lst_ReprintOperation.setCurrentRow(row);
        ListCellLine line = _lcm_lst_ReprintOperation.get(row);
        ...
        {
            // エクスポートクラス生成情報の取得
            ExporterFactory factory = new WmsExporterFactory(httpRequest.getLocale(), (DfkUserInfo)getUserInfo());
            // エクスポートクラスの生成
            exporter = factory.newPrinterExporter(line.getStringValue(KEY_HIDDEN_XML_FILE_NAME), false);
        
            try
            {
                // 再発行を行う
                exporter.rePrint(line.getStringValue(KEY_HIDDEN_FILE_NAME));
            ...
            part11List.add(line.getViewString(KEY_LST_LIST_NAME), "");
            part11List.add(line.getViewString(KEY_LST_PRINTED_ON), "");
            part11List.add(line.getViewString(KEY_LST_PRINTED_IN_SCREEN), "");
            part11Writer.createOperationLog((DfkUserInfo)getUserInfo(), ConvertUtil.objectToInt(EmConstants.OPELOG_CLASS_PRINT), part11List);
            conn.commit();
        }
        catch (Exception ex)
        {
