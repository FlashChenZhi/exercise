 ...
 * セッション管理の画面処理を行います。
 *
 * @version $Revision: 7746 $, $Date: 2010-03-29 16:02:39 +0900 (月, 29 3 2010) $
        ...
        extends SessionManage
{
     ...
     * リスト：セッションID
     */
    private static final int LST_SESSION_ID = 6;
    // DFKLOOK:ここまで修正
	...
	public void page_ConfirmBack(ActionEvent e) throws Exception
	{
		...
		// choose process.
		if(eventSource.startsWith("btn_Station_Stop_Click"))
		{
			// process call.
			btn_Station_Stop_Click_Process(eventSource);
		}
		if(eventSource.startsWith("btn_Logoff_Click"))
		{
			// process call.
			btn_Logoff_Click_Process(eventSource);
		}
	}
	// DFKLOOK end
    ...
    {
        // process call.
        btn_Station_Stop_Click_Process(null);
    }
     ...
     * @return String[]
     */
    protected String[] convertToArray(ArrayList<String> aryParam)
    {
        if (aryParam.size() > 0)
        {
            ...
            throws Exception
    {
        // DFKLOOK:ここから修正
        // プルダウン生成
        PulldownHelper.setPullDown(pul_TerminalName, convertToArray(WmsParam.TERMINALSERVER_NAME));
     ...
     * @throws Exception
     */
    private void btn_Station_Stop_Click_Process(String eventSource)
            throws Exception
    {
        // DFKLOOK start
        ...
        {
            ListCellLine checkline = _lcm_lst_UserList.get(i);
        ...
        {
        	// セッションを切断しますか？
            this.setConfirm("MSG-SM001", false, true);
    		viewState.setString(_KEY_CONFIRMSOURCE, "btn_Station_Stop_Click");
            return;
        }
        // DFKLOOK end
        ...
        try
        {
            ...
            // DFKLOOK:ここから修正
            // モデルからオブジェクトに変更
            int index = lst_UserList.getMaxRows();
            for (int i = 1; i < index; i++)
            {
                lst_UserList.setCurrentRow(i);
                if (lst_UserList.getChecked(1))
                {
                    ...
                    lineparam.set(SessionManageSCHParams.SESSION_ID, lst_UserList.getValue(LST_SESSION_ID));
                    lineparam.set(SessionManageSCHParams.PROCESS_FLAG, SessionManageSCH.PROCESS_DISCONNECT);
     ...
     * @throws Exception
     */
    private void btn_Logoff_Click_Process(String eventSource)
            throws Exception
    {
        // DFKLOOK start
        boolean existEditedRow = false;
        ...
        {
        	// セッションをリセットしますか？
            this.setConfirm("MSG-SM002", false, true);
    		viewState.setString(_KEY_CONFIRMSOURCE, "btn_Logoff_Click");
            return;
        }
        // DFKLOOK end
        ...
        try
        {
            ...
            for (int i = 1; i < index; i++)
            {
                ...
                if (lst_UserList.getChecked(1))
                {
                    ...
                    lineparam.set(SessionManageSCHParams.SERVER_NAME, _pdm_pul_TerminalName.getSelectedValue());
                    lineparam.set(SessionManageSCHParams.SESSION_ID, lst_UserList.getValue(LST_SESSION_ID));
                    lineparam.set(SessionManageSCHParams.PROCESS_FLAG, SessionManageSCH.PROCESS_RESET);
                    inparamList.add(lineparam);
                }
            }
            if (inparamList.size() <= 0)
