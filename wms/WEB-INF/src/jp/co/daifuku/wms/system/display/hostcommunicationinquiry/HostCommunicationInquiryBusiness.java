        ...
        extends HostCommunicationInquiry
{
    ...
    private ListCellModel _lcm_lst_HostCommunicationInquiry;
     ...
     * @throws Exception
     */
    public void lst_HostCommunicationInquiry_Click(ActionEvent e)
            throws Exception
    {
        // DFKLOOK ここから修正
        lst_HostCommunicationInquiry_Click_Process();
        // DFKLOOK ここまで修正
    }
            ...
            throws Exception
    {
        ...
        _lcm_lst_HostCommunicationInquiry = new ListCellModel(lst_HostCommunicationInquiry, LST_HOSTCOMMUNICATIONINQUIRY_KEYS, locale);
            ...
            throws Exception
    {
        ...
        try
        {
            ...
            _pdm_pul_CommunicationData.clear();
            ...
            throws Exception
    {
        ...
        try
        {
            ...
            // load pul_CommunicationData.
            _pdm_pul_CommunicationData.init(conn, DataType.SEND);
            ...
            throws Exception
    {
        ...
        try
        {
            ...
            if (!chk.checkDate(txt_SearchDate.getDate(), txt_ToSearchTime.getTime()))
            {
                message.setMsgResourceKey(chk.getMessage());
                return;
            }
            ...
            {
                // 詳細ボタンを有効に設定
                if (outparams.get(i - 1).getBoolean(HostCommunicationInquirySCHParams.IS_DETAILS))
                {
                    ListCellLine enabledline = _lcm_lst_HostCommunicationInquiry.get(i);
                    lst_HostCommunicationInquiry.setCurrentRow(i);
                    _lcm_lst_HostCommunicationInquiry.set(i, enabledline);
                    lst_HostCommunicationInquiry.setCellEnabled(
                            _lcm_lst_HostCommunicationInquiry.getColumnIndex(KEY_LST_DETAILS), true);
                }
            }
            // DFKLOOK ここまで修正
    ...
    // DFKLOOK ここから修正
    /**
     * リストセル詳細ボタン押下時の処理を行います。
     * 
     * @throws Exception 全ての例外を報告します。
     */
    private void lst_HostCommunicationInquiry_Click_Process()
            throws Exception
    {

        // get active row.
        int row = lst_HostCommunicationInquiry.getActiveRow();
        lst_HostCommunicationInquiry.setCurrentRow(row);
        ListCellLine line = _lcm_lst_HostCommunicationInquiry.get(row);

        // dialog parameters set.
        LstHostErrorParams inparam = new LstHostErrorParams();
        inparam.set(LstHostErrorParams.FILE_NAME, line.getValue(KEY_LST_FILE_NAME));
        inparam.set(LstHostErrorParams.START_DATE, line.getValue(KEY_HIDDEN_START_DATE));
        // DFKLOOK start
        inparam.set(LstHostErrorParams.STATUS, line.getValue(KEY_LST_STATUS));
        // DFKLOOK end

        // show dialog.
        ForwardParameters forwardParam = inparam.toForwardParameters();
        forwardParam.setParameter(_KEY_POPUPSOURCE, "lst_HostCommunicationInquiry_ColumClick");
        redirect("/system/listbox/hosterror/LstHostError.do", forwardParam, "/Progress.do");
    }
