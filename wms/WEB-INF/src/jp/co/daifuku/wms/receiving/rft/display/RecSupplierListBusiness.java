...
import jp.co.daifuku.wms.receiving.rft.datasheet.SupplierListDataSheet;
...
import jp.co.daifuku.wms.receiving.rft.schedule.RecSupplierListSCH;
import jp.co.daifuku.wms.receiving.rft.schedule.RecSupplierListSCHParams;
 ...
 */
public class RecSupplierListBusiness
        extends AbstractHSBusiness
{
    ...
    //------------------------------------------------------------
    /** SCREEN_NAME */
    private static final String SCREEN_NAME = "RecSupplierList";
    ...
    public HSResult page_Load(DataSheetFactory dsf, Locale locale)
    {
        ...
        SupplierListDataSheet supplierListDataSheet = new SupplierListDataSheet(dsf, locale);
        TermInfoDataSheet termInfoDataSheet = new TermInfoDataSheet(dsf, locale);

        Connection conn = null;
        RecSupplierListSCH sch = null;
        try
        {
            // open connection.
            conn = ConnectionManager.getConnection(WMSConstants.DATASOURCE_RFT_NAME);
            sch = new RecSupplierListSCH(conn, this.getClass(), locale);

            // set input parameters.
            RecSupplierListSCHParams inparam = new RecSupplierListSCHParams();
            inparam.set(RecSupplierListSCHParams.TERMINAL_NO, termInfoDataSheet.getValue(TermInfoDataSheet.TERMINAL_NO));
            inparam.set(RecSupplierListSCHParams.RECEIVING_ITF_TO_JAN, termInfoDataSheet.getValue(TermInfoDataSheet.RECEIVING_ITF_TO_JAN));
            inparam.set(RecSupplierListSCHParams.USER_ID, commonDataSheet.getValue(CommonDataSheet.USER_ID));
            inparam.set(RecSupplierListSCHParams.CONSIGNOR_CODE, receivingInDataSheet.getValue(ReceivingInDataSheet.CONSIGNOR_CODE));
            inparam.set(RecSupplierListSCHParams.ITEM_CODE, receivingInDataSheet.getValue(ReceivingInDataSheet.ITEM_CODE));
            inparam.set(RecSupplierListSCHParams.PLAN_DATE, receivingInDataSheet.getValue(ReceivingInDataSheet.PLAN_DATE));

            // SCH call.
            List<Params> outparams = sch.query(inparam);

            // set output parameters.
            supplierListDataSheet.setValue(DataSheetModel.SIZE, outparams.size());
            for (int i = 0; i < outparams.size(); i++)
            {
                Params outparam = outparams.get(i);
                supplierListDataSheet.setValue(SupplierListDataSheet.SUPPLIER_CODE, outparam.get(RecSupplierListSCHParams.SUPPLIER_CODE), i);
                supplierListDataSheet.setValue(SupplierListDataSheet.SUPPLIER_NAME, outparam.get(RecSupplierListSCHParams.SUPPLIER_NAME), i);
            }

            // initialize DataSheet view.
            supplierListDataSheet.initView();
    ...
    public HSResult ent_Click(DataSheetFactory dsf, Locale locale)
    {
        ...
        try
        {
            ...
            inparam.set(RecStartSCHParams.SUPPLIER_CODE, supplierListDataSheet.getValue(SupplierListDataSheet.SUPPLIER_CODE));
