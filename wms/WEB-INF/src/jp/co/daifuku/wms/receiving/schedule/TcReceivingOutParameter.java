...
/**
 * <CODE>TcReceivingOutParameter</CODE>クラスは、入荷パッケージ内のスケジュール→画面間のパラメータの受渡しに使用します。<BR>
 * このクラスでは入荷パッケージの各画面で使用される項目を保持します。使用する項目は画面によって異なります。<BR>
 * <BR>
 * <DIR>
 * <CODE>TcReceivingOutParameter</CODE>クラスが保持する項目<BR>
 * <BR>
 *     予定一意キー<BR>
 *     取込単位キー<BR>
 *     設定単位キー<BR>
 ...
 * @author  $Author: admin $
 */
public class TcReceivingOutParameter
        extends OutParameter
{
     ...
     * @since 2008/4/2 add by Softecs.
     */
    public void setBundleQty(int bundleEnteringQty)
    {
        _bundleQty = bundleEnteringQty;
    }
     ...
     */
    public boolean setKeys(ReceivingOutParameter workInfo)
    {
        boolean isEqual = true;
        // 予定日
        String wkstr = workInfo.getReceivingDay();
        if (!equals(getPlanDay(), wkstr))
        {
            setPlanDay(wkstr);
            isEqual &= false;
        }
        // 仕入先
        wkstr = workInfo.getSupplierCode();
        ...
        // 伝票No
        wkstr = workInfo.getTicketNo();
        if (!equals(getTicketNo(), wkstr))
        {
            setTicketNo(wkstr);
            isEqual &= false;
        }
