// $Id: ViewStateKeys.java 3208 2009-03-02 05:42:52Z arai $
package jp.co.daifuku.wms.crossdock.display.planregist;

/*
 * Copyright(c) 2000-2007 DAIFUKU Co.,Ltd. All Rights Reserved.
 *
 * This software is the proprietary information of DAIFUKU Co.,Ltd.
 * Use is subject to license terms.
 */

/**
 * ViewStateのキーを定義するクラスです。
 * ここに具体的なクラスの説明を記述して下さい。
 *
 * @version $Revision: 3208 $, $Date: 2009-03-02 14:42:52 +0900 (月, 02 3 2009) $
 * @author  BusiTune 1.0 Generator.
 * @author  Last commit: $Author: arai $
 */
interface ViewStateKeys
{
    //------------------------------------------------------------
    // fields (upper case only)
    //------------------------------------------------------------
    //
    public static final String BATCH_NO = "BATCH_NO";
    //
    public static final String CASE_ITF = "CASE_ITF";
    //
    public static final String CASE_PACK = "CASE_PACK";
    //
    public static final String CONSIGNOR_CODE = "CONSIGNOR_CODE";
    //
    public static final String ITEM_CODE = "ITEM_CODE";
    //
    public static final String ITEM_NAME = "ITEM_NAME";
    //
    public static final String JAN_CODE = "JAN_CODE";
    //
    public static final String LINE_NO = "LINE_NO";
    //
    public static final String LOT_NO = "LOT_NO";
    //
    public static final String MAP_LINE_NO = "MAP_LINE_NO";
    //
    public static final String MASTER = "MASTER";
    //
    public static final String NEXT_LINE_NO = "NEXT_LINE_NO";
    //
    public static final String PLAN_DATE = "PLAN_DATE";
    //
    public static final String RECEIVING_SLIP_NUMBER = "RECEIVING_SLIP_NUMBER";
    //
    public static final String SUPPLIER_CODE = "SUPPLIER_CODE";
    //
    public static final String SUPPLIER_NAME = "SUPPLIER_NAME";

    //------------------------------------------------------------
    // class variables (prefix '$')
    //------------------------------------------------------------

    //------------------------------------------------------------
    // instance variables (prefix '_')
    //------------------------------------------------------------

    //------------------------------------------------------------
    // constructors
    //------------------------------------------------------------

    //------------------------------------------------------------
    // public methods
    //------------------------------------------------------------

    //------------------------------------------------------------
    // accessor methods
    //------------------------------------------------------------

    //------------------------------------------------------------
    // package methods
    //------------------------------------------------------------

    //------------------------------------------------------------
    // protected methods
    //------------------------------------------------------------

    //------------------------------------------------------------
    // private methods
    //------------------------------------------------------------

    //------------------------------------------------------------
    // utility methods
    //------------------------------------------------------------

}
//end of class
