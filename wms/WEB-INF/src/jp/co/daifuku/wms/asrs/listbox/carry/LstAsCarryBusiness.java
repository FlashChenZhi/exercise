// $Id: LstAsCarryBusiness.java 7996 2011-07-06 00:52:24Z kitamaki $
        ...
        extends LstAsCarry
{
     ...
     * 検索条件表示行番号 : 作業場
     */
    private static final int CONDITION_WORK_PLACE = 1;
    
    /**
     * 検索条件表示行番号 : ステーション
     */
    private static final int CONDITION_STAION = 2;
    ...
    private ListCellModel _lcm_lst_RestWorkIncludeButtonList;
     ...
     * <BR>
     * @param param 表示検索条件
     * @param sc ステーションコントローラ
     * @throws Exception 全ての例外を報告します。
     */
    protected void setSearchlist(LstAsCarryParams param, StationController sc)
            throws Exception
    {
        // 作業場
        lst_SearchCondition.addRow();
        lst_SearchCondition.setCurrentRow(CONDITION_WORK_PLACE);
        lst_SearchCondition.setValue(S_LIST1, DisplayText.getText("LBL-W0085"));
        String stName = "";
        if (WmsParam.ALL_STATION.equals(param.getString(LstAsCarryParams.WORK_PLACE_KEY)))
        {
            stName = param.getString(LstAsCarryParams.WORK_PLACE_KEY)+ "：" + DisplayText.getText("CMB-W0040");
            lst_SearchCondition.setValue(S_LIST2, stName);
        }
        else
        {
            lst_SearchCondition.setValue(S_LIST2, sc.getAsStationName(param.getString(LstAsCarryParams.WORK_PLACE_KEY)));
        }

        // ステーション
        lst_SearchCondition.addRow();
        lst_SearchCondition.setCurrentRow(CONDITION_STAION);
        lst_SearchCondition.setValue(S_LIST1, DisplayText.getText("LBL-W0362"));
        stName = "";
        if (WmsParam.ALL_STATION.equals(param.getString(LstAsCarryParams.STATION_NO_KEY)))
        {
            stName = param.getString(LstAsCarryParams.STATION_NO_KEY) + "：" + DisplayText.getText("CMB-W0028");
            lst_SearchCondition.setValue(S_LIST2, stName);
        }
        else
        {
            lst_SearchCondition.setValue(S_LIST2, sc.getAsStationName(param.getString(LstAsCarryParams.STATION_NO_KEY)));
        }
    }
            ...
            throws Exception
    {
        ...
        _lcm_lst_RestWorkIncludeButtonList = new ListCellModel(lst_RestWorkIncludeButtonList, LST_RESTWORKINCLUDEBUTTONLIST_KEYS, locale);
            ...
            throws Exception
    {
        ...
        line.addToolTip("LBL-W0276", KEY_HIDDEN_RETRIEVAL_DETAIL_NAME);
        line.addToolTip("LBL-W0079", KEY_HIDDEN_WORK_TYPE_NAME);
            ...
            throws Exception
    {
        ...
        try
        {
            ...
            //DFKLOOK:ここから修正
            if (StringUtil.isBlank(requestParam.get(LstAsCarryParams.WORK_PLACE_KEY))
                    || StringUtil.isBlank(requestParam.get(LstAsCarryParams.STATION_NO_KEY)))
            {
                // 対象データはありませんでした。
            ...
            this.viewState.setString(ViewStateKeys.WORK_PLACE_KEY, requestParam.getString(LstAsCarryParams.WORK_PLACE_KEY));
            this.viewState.setString(ViewStateKeys.STATION_NO_KEY, requestParam.getString(LstAsCarryParams.STATION_NO_KEY));
            this.viewState.setString(ViewStateKeys.TERMINAL_NO_KEY, requestParam.getString(LstAsCarryParams.TERMINAL_NO_KEY));
            StationController sc = new StationController(conn, this.getClass());
            setSearchlist(requestParam, sc);
            //DFKLOOK:ここまで修正
