// $Id: AsWorkMntBusiness.java 7851 2010-04-21 08:49:56Z shibamoto $
...
import jp.co.daifuku.wms.asrs.listbox.faworkdetail.FaLstAsWorkDetailParams;
        ...
        extends AsWorkMnt
{
            ...
            throws Exception
    {
		...
		// DFKLOOK start
        // choose process.
        if (eventSource.startsWith("btn_Print_Click"))
        {
            // process call.
            btn_Print_Click_Process(eventSource);
        }
    ...
    //DFKLOOK:ここから修正
    /**
     * ステーション名称を取得し、不要情報は省きます。
     * 
     * @param sCtrl ステーション情報コントローラ
     * @param stationNo ステーションNo.
     * @return String 不要情報を取り除いたステーション名称
     * @throws Exception
     */
    protected String getStationName(StationController sCtrl, String stationNo)
            throws Exception
    {
        // 全ステーションの場合
        ...
        String station = "";
        station = sCtrl.getAsStationName(stationNo);

        if (!StringUtil.isBlank(station))
        {
            station = station.substring(5, station.length());
        }
        return station;
    }
    //DFKLOOK:ここから修正
            ...
            throws Exception
    {
        ...
        try
        {
            ...
            // load pul_WorkPlace.
            //DFKLOOK:ここから修正
            _pdm_pul_WorkPlace.init(conn, StationType.WORK_MNT, WmsParam.ALL_STATION, Distribution.ALL);
            //DFKLOOK:ここまで修正
            ...
            throws Exception
    {
        ...
        // DFKLOOK ここから修正
        // クリアボタン押下時確認ダイアログ "取消しますか？"
        btn_Clear.setBeforeConfirm("MSG-W0002");
        // DFKLOOK ここまで修正
            ...
            throws Exception
    {
        ...
        // DFKLOOK ここから修正
        inparam.set(LstAsCarryParams.TERMINAL_NO_KEY, ((DfkUserInfo)this.getUserInfo()).getTerminalNumber());
        // DFKLOOK ここまで修正
     ...
     */
    // DFKLOOK start
    private void btn_Print_Click_Process(String eventSource)
            throws Exception
	// DFKLOOK end
    {
        // get locale.
        ...
        try
        {
            ...
            //DFKLOOK:ここから修正
            inparam.set(AsWorkMntExporterDASCHParams.TERMINAL_NO, ((DfkUserInfo)this.getUserInfo()).getTerminalNumber());
            //DFKLOOK:ここまで修正

            // check count.
            int count = dasch.count(inparam);
			// DFKLOOK start
            if (StringUtil.isBlank(eventSource) && count > 0)
			// DFKLOOK end
            {
            ...
            //DFKLOOK:ここから修正
            StationController sCtrl = new StationController(conn, this.getClass());
            //DFKLOOK:ここまで修正
            ...
            while (dasch.next())
            {
                ...
                //DFKLOOK:ここから修正
                expparam.set(CarryListParams.STATION_NAME, getStationName(sCtrl, _pdm_pul_Station.getSelectedValue().toString()));
                //DFKLOOK:ここまで修正
                ...
                // DFKLOOK start
                // はじめに出てくる":"まで(STNo.)と、あと(ST名称)に分け、名称をセットする。
                expparam.set(CarryListParams.WORK_AREA, pul_WorkPlace.getSelectedItem().getText().split(":", 2)[1]);
                // DFKLOOK end
	...
	// DFKLOOK end
    {
        ...
        try
        {
            ...
            // DFKLOOK ここから修正
            if (rdo_ASRSProcess_NoIndication.getChecked())
            {
                inparam.set(AsWorkMntSCHParams.ASRS_PROCESS, AsrsInParameter.PROCESS_STATUS_UNINSTRUCT);
            }
            else if (rdo_ASRSProcess_Finished.getChecked())
            {
                inparam.set(AsWorkMntSCHParams.ASRS_PROCESS, AsrsInParameter.PROCESS_STATUS_COMPLETE);
            }
            else if (rdo_ASRSProcess_CancelAllocate.getChecked())
            {
                inparam.set(AsWorkMntSCHParams.ASRS_PROCESS, AsrsInParameter.PROCESS_STATUS_CANCEL_ALLOCATE);
            }
            else if (rdo_ASRSProcess_TrackingDelete.getChecked())
            {
                inparam.set(AsWorkMntSCHParams.ASRS_PROCESS, AsrsInParameter.PROCESS_STATUS_TRACKING_DELETE);
            }
            // DFKLOOK ここまで修正
            ...
            throws Exception
    {
        ...
        try
        {
            // open connection.
            conn = ConnectionManager.getRequestConnection(this);
            WarenaviSystemController wCtrl = new WarenaviSystemController(conn, this.getClass());
            if (wCtrl.isFaDaEnabled())
            {
                // dialog parameters set.
                FaLstAsWorkDetailParams inparam = new FaLstAsWorkDetailParams();
                inparam.set(FaLstAsWorkDetailParams.WORK_NO, txt_WorkNo.getValue());
                inparam.set(FaLstAsWorkDetailParams.CARRY_KEY, txt_CarryingKey.getValue());

                // show dialog.
                ForwardParameters forwardParam = inparam.toForwardParameters();
                forwardParam.setParameter(_KEY_POPUPSOURCE, "btn_Details_Click");
                redirect("/asrs/listbox/faworkdetail/FaLstAsWorkDetail.do", forwardParam, "/Progress.do");
            }
            else
            {
                // dialog parameters set.
