...
import static jp.co.daifuku.wms.sort.schedule.SortCompleteSCHParams.*;
...
import jp.co.daifuku.wms.sort.operator.SortOperator;

/**
 * 仕分一括確定のスケジュール処理を行います。
 * 
 * @version $Revision: 1.2 $, $Date: 2009/02/24 02:43:12 $
 * @author  BusiTune 1.0 Generator.
 * @author  Last commit: $Author: ose $
 */
public class SortCompleteSCH
        extends AbstractSCH
{
    //------------------------------------------------------------
     ...
     */
    public SortCompleteSCH(Connection conn, Class parent, Locale locale, DfkUserInfo ui) throws CommonException
    {
    ...
    //------------------------------------------------------------
    /**
     * 画面から入力された内容をパラメータとして受け取り、仕分一括確定処理のスケジュールを開始します。<BR>
     * スケジュールが正常終了した場合はtrue、失敗した場合はfalseを返します。
     *
     * @param startParams 設定内容を持つ<CODE>SortInParameter</CODE>クラスのインスタンスの配列。 <BR>
            ...
            throws CommonException
    {
        ...
        try
        {
            ...
            SortCompleteSCHParams startParam = (SortCompleteSCHParams)startParams;

            SortInParameter sParam = new SortInParameter(getWmsUserInfo());
            ...
            // 仕入先コード
            sParam.setCustomerCode(startParam.getString(CUSTOMER_CODE));
            // バッチNo.
            sParam.setBatchNo(startParam.getString(BATCH_NO));
            // 商品コード
            sParam.setItemCode(startParam.getString(ITEM_CODE));
            ...
            // オペレータ
            SortOperator operator = new SortOperator(getConnection(), getClass());

            // 一括確定処理
            OutParameter outParam = operator.webComplete((SortInParameter)sParam);
