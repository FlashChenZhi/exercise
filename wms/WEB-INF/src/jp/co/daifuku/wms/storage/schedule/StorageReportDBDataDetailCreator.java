// $Id: StorageReportDBDataDetailCreator.java 7735 2010-03-26 06:22:49Z okayama $
 ...
 */
public class StorageReportDBDataDetailCreator
        extends StorageReportDBDataCreator
{
    //------------------------------------------------------------
     ...
     */
    public StorageReportDBDataDetailCreator(Connection conn, Connection customerConn)
    {
     ...
     */
    public StorageReportDBDataDetailCreator(Connection conn, Connection customerConn, Class caller)
    {
    ...
    //------------------------------------------------------------
    /**
     * 入庫実績 予定単位（明細）のデータ報告作成処理を行います。<BR>
     ...
     * @throws ReadWriteException ファイルアクセスでエラーが発生した場合に通知されます。<BR>
     */
    protected boolean csvWrite(FileHandler handler, ReportStorage rStorageEntity, HostSend hostSend)
            throws ReadWriteException
    {
        // 入庫実績報告CSVファイルの出力内容を編集します。
        rStorageEntity.setValue(ReportStorage.CANCEL_FLAG, SystemDefine.CANCEL_FLAG_NORMAL);
        rStorageEntity.setValue(ReportStorage.PLAN_DAY, hostSend.getPlanDay());
        rStorageEntity.setValue(ReportStorage.RECEIVE_TICKET_NO, hostSend.getReceiveTicketNo());
        rStorageEntity.setValue(ReportStorage.RECEIVE_LINE_NO, new BigDecimal(hostSend.getReceiveLineNo()));
        rStorageEntity.setValue(ReportStorage.BRANCH_NO, new BigDecimal(hostSend.getReceiveBranchNo()));
        rStorageEntity.setValue(ReportStorage.ITEM_CODE, hostSend.getItemCode());
        rStorageEntity.setValue(ReportStorage.ITEM_NAME, hostSend.getItemName());
        rStorageEntity.setValue(ReportStorage.JAN, hostSend.getJan());
        rStorageEntity.setValue(ReportStorage.ITF, hostSend.getItf());
        rStorageEntity.setValue(ReportStorage.ENTERING_QTY, new BigDecimal(hostSend.getEnteringQty()));
        rStorageEntity.setValue(ReportStorage.PLAN_LOT_NO, hostSend.getPlanLotNo());
        rStorageEntity.setValue(ReportStorage.PLAN_QTY, new BigDecimal(hostSend.getPlanQty()));
        rStorageEntity.setValue(ReportStorage.PLAN_AREA_NO, hostSend.getPlanAreaNo());
        rStorageEntity.setValue(ReportStorage.PLAN_LOCATION_NO, hostSend.getPlanLocationNo());
        rStorageEntity.setValue(ReportStorage.RESULT_QTY, new BigDecimal(hostSend.getResultQty()));
        rStorageEntity.setValue(ReportStorage.WORK_DAY, hostSend.getWorkDay());

        // 明細の場合は、明細部分を編集します。
        rStorageEntity.setValue(ReportStorage.RESULT_AREA_NO, hostSend.getResultAreaNo());
        rStorageEntity.setValue(ReportStorage.RESULT_LOCATION_NO, hostSend.getResultLocationNo());
        rStorageEntity.setValue(ReportStorage.RESULT_LOT_NO, hostSend.getResultLotNo());
        rStorageEntity.setValue(ReportStorage.USER_ID, hostSend.getUserId());
        rStorageEntity.setValue(ReportStorage.TERMINAL_NO, hostSend.getTerminalNo());
    ...
    {
        return "$Id: StorageReportDBDataDetailCreator.java 7735 2010-03-26 06:22:49Z okayama $";
    }
}
