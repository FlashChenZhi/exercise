// $Id: DataLoadSCH.java 7905 2010-05-06 05:12:47Z kumano $
...
import static jp.co.daifuku.pcart.system.schedule.DataLoadSCHParams.*;
...
/**
 * データ取込（ファイル）のスケジュール処理を行います。
 *
 * @version $Revision: 7905 $, $Date:: 2010-05-06 14:12:47 +0900#$
 * @author  BusiTune 1.0 Generator.
 * @author  Last commit: $Author: kumano $
 */
public class DataLoadSCH
        extends AbstractSCH
{
    //------------------------------------------------------------
     ...
     */
    public DataLoadSCH(Connection conn, Class parent, Locale locale, DfkUserInfo ui) throws CommonException
    {
            ...
            throws CommonException
    {
        ...
        if (sysController.hasPCTMasterPack())
        {
            ...
            for (ExchangeEnvironment exEnv : exEnvs)
            {
                ...
                // ファイル数
                int count = getLoadFileCount(exEnv.getClassName(), exEnv.getJobType());
                param.set(FILE_COUNT, count);
                // クラス名
        ...
        try
        {
            ...
            for (ScheduleParams p : ps)
            {
                ...
                if (p.getBoolean(SELECT))
                {
                    ...
                    // コンストラクタを取得
                    constructor = loaderClass.getConstructor(String.class, DfkUserInfo.class, Locale.class);
                    // インスタンス生成
                    obj = constructor.newInstance(p.getString(DATA_TYPE), getUserInfo(), getLocale());
                    // execute()呼び出し
                ...
                // データ数
                outParam.set(FILE_COUNT, getLoadFileCount(p.getString(CLASS_NAME), p.getString(DATA_TYPE)));
                // クラス名
     ...
     */
    private int getLoadFileCount(String className, String exchangeJob)
            throws CommonException
    {
        // コンストラクタクラス
        Constructor constructor = null;
        ...
        try
        {
            ...
            constructor = loaderClass.getConstructor(String.class, DfkUserInfo.class, Locale.class);
            // インスタンス生成
            Object obj = constructor.newInstance(exchangeJob, getUserInfo(), getLocale());
            // getFileCount()呼び出し
