// $Id: WorkMntSCH.java 4328 2009-05-26 01:25:27Z okayama $
 ...
 */
import static jp.co.daifuku.pcart.retrieval.schedule.WorkMntSCHParams.*;
...
/**
 * 作業メンテナンスのスケジュール処理を行います。
 *
 * @version $Revision: 4328 $, $Date:: 2009-05-26 10:25:27 +0900#$
 * @author  BusiTune 1.0 Generator.
 * @author  Last commit: $Author: okayama $
 */
public class WorkMntSCH
        extends AbstractSCH
{
    //------------------------------------------------------------
     ...
     */
    public WorkMntSCH(Connection conn, Class parent, Locale locale, DfkUserInfo ui) throws CommonException
    {
    ...
    protected SearchKey createSearchKey(ScheduleParams p)
    {
        ...
        {
            wSearchKey.setPlanDay(WmsFormatter.toParamDate(p.getDate(PLAN_DATE)));
        }
        // バッチNo.
        if (!StringUtil.isBlank(p.getString(BATCH_NO)))
        {
            wSearchKey.setBatchNo(p.getString(BATCH_NO));
        }
        // バッチSeqNo.
        if (!StringUtil.isBlank(p.getString(BATCH_SEQ_NO)))
        {
            wSearchKey.setBatchSeqNo(p.getString(BATCH_SEQ_NO));
        }
        // オーダーNo.
        ...
        if (!StringUtil.isBlank(p.getString(AREA_NO)))
        {
            ...
            {
                wSearchKey.setPlanAreaNo(p.getString(AREA_NO));
            }
        }
        // 状態フラグ
        ...
        // 商品コード順
        if (PCTRetrievalInParameter.ORDERING_ITEM_CODE.equals(p.getString(DISP_ORDER)))
        {
            // 昇順
            // バッチNo.
            wSearchKey.setBatchNoOrder(true);
            // バッチSeqNo.
            wSearchKey.setBatchSeqNoOrder(true);
            // 出荷先コード
            wSearchKey.setCustomerCodeOrder(true);
            // 商品コード
            wSearchKey.setItemCodeOrder(true);
            // エリアNo.
            wSearchKey.setPlanAreaNoOrder(true);
            // ゾーンNo.
            wSearchKey.setPlanZoneNoOrder(true);
            // 棚No.
            wSearchKey.setPlanLocationNoOrder(true);
            // 伝票No.
            wSearchKey.setShipTicketNoOrder(true);
        }
        // オーダーNo.順
        else
        {
            ...
            wSearchKey.setCustomerCodeOrder(true);
            // オーダーNo.
            wSearchKey.setResultOrderNoOrder(true);
            // 商品コード
            wSearchKey.setItemCodeOrder(true);
            // エリアNo.
            wSearchKey.setPlanAreaNoOrder(true);
            ...
            throws CommonException
    {
        ...
        for (int i = 0; i < workInfoEntity.length; i++)
        {
            ...
            // 最終更新日時
            param.set(LAST_UPDATE_DATE, WmsFormatter.toParamDateTime(workInfoEntity[i].getLastUpdateDate(), 3));
