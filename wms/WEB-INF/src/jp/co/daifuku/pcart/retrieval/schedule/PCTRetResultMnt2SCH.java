// $Id: PCTRetResultMnt2SCH.java 4270 2009-05-13 03:57:38Z okayama $
...
import static jp.co.daifuku.pcart.retrieval.schedule.PCTRetResultMnt2SCHParams.*;
        ...
        extends AbstractSCH
{
     ...
     */
    public PCTRetResultMnt2SCH(Connection conn, Class parent, Locale locale, DfkUserInfo ui) throws CommonException
    {
    ...
    protected SearchKey createSearchKey(ScheduleParams p)
    {
        ...
        // オーダーNo.
        wSearchKey.setCollect(PCTRetWorkInfo.ORDER_NO);
        // 予定オーダーNo.
        ...
        // 昇順
        // 予定日
        wSearchKey.setPlanDayOrder(true);
        // バッチNo.
        wSearchKey.setBatchNoOrder(true);
        // バッチSeqNo.
        wSearchKey.setBatchSeqNoOrder(true);
        // 得意先コード
        wSearchKey.setRegularCustomerCodeOrder(true);
        // 出荷先コード
            ...
            throws CommonException
    {
        ...
        for (int i = 0; i < workInfoEntity.length; i++)
        {
            ...
            // 予定日
            param.set(PLAN_DAY, WmsFormatter.toDate(workInfoEntity[i].getPlanDay()));
            // バッチSeqNo.
