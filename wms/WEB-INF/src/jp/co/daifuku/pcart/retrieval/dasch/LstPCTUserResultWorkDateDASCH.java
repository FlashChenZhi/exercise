// $Id: LstPCTUserResultWorkDateDASCH.java 5739 2009-11-12 13:28:36Z kumano $
...
import static jp.co.daifuku.pcart.retrieval.dasch.LstPCTUserResultWorkDateDASCHParams.*;
...
import jp.co.daifuku.pcart.retrieval.dbhandler.PCTUserResultWorkDateFinder;
...
/**
 * 作業日別実績照会に必要なリストボックスや帳票の検索処理を行います。
 *
 * @version $Revision: 5739 $, $Date:: 2009-11-12 22:28:36 +0900#$
 * @author  BusiTune 1.0 Generator.
 * @author  Last commit: $Author: kumano $
 */
public class LstPCTUserResultWorkDateDASCH
        extends AbstractWmsDASCH
{
     ...
     * @param ui ユーザ情報
     */
    public LstPCTUserResultWorkDateDASCH(Connection conn, Class parent, Locale locale, DfkUserInfo ui)
    {
            ...
            throws CommonException
    {
        ...
        _finder = new PCTUserResultWorkDateFinder(getConnection());
    ...
    protected void convertDetail(Params ioParam, PCTPickingResult inResult)
    {
        try
        {
            ...
            // 作業日
            ioParam.set(WORKDAY, WmsFormatter.toDate(inResult.getWorkDay()));
            // 生産率
    ...
    protected void convertAvg(Params ioParam, PCTPickingResult inResult, int count)
    {
        try
        {
            ...
            if (inResult.getWorkTime() > 0 && count > 0)
            {
                ioParam.set(AVG_TERMINAL_COUNT, formatNumber(tmpInt / count));
            }
            else
            {
                ioParam.set(AVG_TERMINAL_COUNT, formatNumber(0));
            }
            // 作業時間(延べ)(平均)
            if (inResult.getWorkTime() > 0 && count > 0)
            {
