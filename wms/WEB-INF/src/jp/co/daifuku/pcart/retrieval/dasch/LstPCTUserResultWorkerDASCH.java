// $Id: LstPCTUserResultWorkerDASCH.java 5739 2009-11-12 13:28:36Z kumano $
...
import jp.co.daifuku.pcart.retrieval.dbhandler.PCTUserResultWorkerFinder;
 ...
 */
public class LstPCTUserResultWorkerDASCH
        extends AbstractWmsDASCH
{
     ...
     * @param ui ユーザ情報
     */
    public LstPCTUserResultWorkerDASCH(Connection conn, Class parent, Locale locale, DfkUserInfo ui)
    {
            ...
            throws CommonException
    {
        ...
        _finder = new PCTUserResultWorkerFinder(getConnection());
    ...
    protected void convertDetail(Params ioParam, PCTPickingResult inResult)
    {
        try
        {
            ...
            // 作業者
            ioParam.set(USER_ID, inResult.getUserId());
            // 作業者名称
            ioParam.set(USER_NAME, inResult.getUserName());
            // レベル
            
            String level = "";
            if (PCTRetrievalInParameter.LEVEL_A.equals((String)inResult.getValue(PCTUserResultOrderInfo.LEVEL)))
            {
                level = DispResources.getText("LBL-P0247");
            }
            if (PCTRetrievalInParameter.LEVEL_B.equals((String)inResult.getValue(PCTUserResultOrderInfo.LEVEL)))
            {
                level = DispResources.getText("LBL-P0248");
            }
            if (PCTRetrievalInParameter.LEVEL_C.equals((String)inResult.getValue(PCTUserResultOrderInfo.LEVEL)))
            {
                level = DispResources.getText("LBL-P0249");
            }
            ioParam.set(LEVEL, level);
            
            //ioParam.set(LEVEL, inResult.getRank());
            // ランク
            ioParam.set(RANK, (String)inResult.getValue(PCTUserResultOrderInfo.RANK, ""));
            // 生産率
            ...
            // 総稼動時間
            ioParam.set(ALL_OPERATE_TIME, WmsFormatter.toDispHourMinute(inResult.getOperateTime()));
            // 実稼動時間
            ioParam.set(REAL_OPERATE_TIME, WmsFormatter.toDispHourMinute((inResult.getOperateTime()
                    - inResult.getBatteryChangeTime() - inResult.getBreakTime())));
            // 集品時間
            ioParam.set(COLLECT_TIME, WmsFormatter.toDispHourMinute(inResult.getRealWorkTime()));
            // 中断時間
            ioParam.set(STOP_TIME, WmsFormatter.toDispHourMinute(inResult.getBatteryChangeTime()
                    + inResult.getBreakTime()));
            // 作業時間
    ...
    protected void convertAvg(Params ioParam, PCTPickingResult inResult, int count)
    {
        try
        {
             ...
             */
            // ランク(平均)はB固定
            ioParam.set(AVG_RANK, DispResources.getText("LBL-P0088"));
            // 生産率(平均)は100%固定
            BigDecimal rateAvg = new BigDecimal(100);
            ioParam.set(AVG_PRODUCTION_RATE, WmsFormatter.toProductionRate(rateAvg.doubleValue()));

            // 総稼動時間(平均)
            if (inResult.getOperateTime() > 0 && count > 0)
            {
                ioParam.set(AVG_ALL_OPERATE_TIME, WmsFormatter.toDispHourMinute(inResult.getOperateTime() / count));
            }
            else
            {
                ioParam.set(AVG_ALL_OPERATE_TIME, WmsFormatter.toDispHourMinute(inResult.getOperateTime()));
            }
            // 実稼動時間(平均)
            if ((inResult.getOperateTime() - inResult.getBatteryChangeTime() - inResult.getBreakTime()) > 0
                    && count > 0)
            {
                ioParam.set(AVG_REAL_OPERATE_TIME, WmsFormatter.toDispHourMinute((inResult.getOperateTime()
                        - inResult.getBatteryChangeTime() - inResult.getBreakTime())
                        / count));
            }
            else
            ...
            // 集品時間(平均)
            if (inResult.getRealWorkTime() > 0 && count > 0)
            {
                ioParam.set(AVG_COLLECT_TIME, WmsFormatter.toDispHourMinute(inResult.getRealWorkTime() / count));
            }
            else
            {
                ioParam.set(AVG_COLLECT_TIME, WmsFormatter.toDispHourMinute(inResult.getRealWorkTime()));
            }
            // 中断時間(平均)
            if ((inResult.getBatteryChangeTime() + inResult.getBreakTime()) > 0 && count > 0)
            {
                ioParam.set(AVG_STOP_TIME,
                        WmsFormatter.toDispHourMinute((inResult.getBatteryChangeTime() + inResult.getBreakTime())
                                / count));
            }
            else
            {
                ioParam.set(AVG_STOP_TIME, WmsFormatter.toDispHourMinute(inResult.getBatteryChangeTime()));
            }
            // 作業時間(平均)
            ...
            // オーダー数(平均)
            tmpInt = inResult.getBigDecimal(PCTUserResultOrderInfo.ORDER_TOTAL, new BigDecimal(0)).intValue();
            if (tmpInt > 0 && count > 0)
            {
            ...
            // オーダー数/H(平均)
            tmpDec = inResult.getBigDecimal(PCTUserResultOrderInfo.ORDER_HOUR, new BigDecimal(0));
            ...
            // 箱/オーダー(平均)
            tmpDec = inResult.getBigDecimal(PCTUserResultOrderInfo.BOX_ORDER, new BigDecimal(0));
            if (tmpDec.doubleValue() > 0 && count > 0)
            {
                tmpQty = tmpDec.divide(new BigDecimal(count), 1, BigDecimal.ROUND_HALF_UP);
            }
            ...
            // 行/オーダー(平均)
            tmpDec = inResult.getBigDecimal(PCTUserResultOrderInfo.LINE_ORDER, new BigDecimal(0));
            if (tmpDec.doubleValue() > 0 && count > 0)
            {
                tmpQty = tmpDec.divide(new BigDecimal(count), 1, BigDecimal.ROUND_HALF_UP);
            }
