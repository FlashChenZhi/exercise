...
import static jp.co.daifuku.pcart.retrieval.dasch.LstCustomerDASCHParams.*;
        ...
        extends AbstractWmsDASCH
{
    ...
    //------------------------------------------------------------
    /** 保存用のフィールド 出荷先コード(<code>CONSIGNOR_CODE</code>) */
    private FieldName _CUSTOMER_CODE = new FieldName("", "CUSTOMER_CODE");

    /** 保存用のフィールド 出荷先名称(<code>CONSIGNOR_NAME</code>) */
    ...
    {
        //  エンティティ
        Entity[] ents = (Entity[])_finder.getEntities(1);
        // パラメータ配列の生成
        Params p = new Params();
        ...
        {
            // 出荷先コード
            p.set(CUSTOMER_CODE, ent.getValue(_CUSTOMER_CODE));
            // 出荷先名称
            ...
            throws CommonException
    {
        ...
        if (isSet)
        {
            // 取得キーのセット
            // 出荷先コード
            key.setCollect(PCTRetPlan.CUSTOMER_CODE, "", _CUSTOMER_CODE);
            // 出荷先名称
            ...
            throws CommonException
    {
        ...
        if (isSet)
        {
            // 取得キーのセット
            // 出荷先コード
            key.setCollect(PCTRetWorkInfo.CUSTOMER_CODE, "", _CUSTOMER_CODE);
            // 出荷先名称
            key.setCollect(PCTRetPlan.CUSTOMER_NAME, "MAX", _CUSTOMER_NAME);
            ...
            throws CommonException
    {
        ...
        if (isSet)
        {
            // 取得キーのセット
            // 出荷先コード
            key.setCollect(PCTRetResult.CUSTOMER_CODE, "MAX", _CUSTOMER_CODE);
            // 出荷先名称
            key.setCollect(PCTRetResult.CUSTOMER_NAME, "MAX", _CUSTOMER_NAME);
